                              ...
                              fluidPage(theme = shinytheme("superhero"), 
                                    fluidRow(
                                      ...
                                      column(4,h3("Time 0"),
                                             ...
                                             numericInput("DDS", label="Direct digit span", value=2 ,min =0, max=16, step=1, width='100px'),
                                             ...
                                             numericInput("SF", label="Category fluency", value=21 ,min =0, max=100, step=1, width='100px')
                                                            ),
                                     column(4,h3("Time 1"),
                                            ...
                                            numericInput("DDS.T1", label="Direct digit span", value=2 ,min =0, max=16, step=1, width='100px'),
                                            ...
                                            numericInput("SF.T1", label="Category fluency", value=21 ,min =0, max=100, step=1, width='100px')
                                                            )),
                                    fluidRow(
...
server <- function(input, output) {
    ...
    output$table0 <-renderDT ({
        ...
        label.tasks<-c("SRT average recall","SRT delayed recall", "Digit symbol", 
                 "Direct digit span", "Backward digit span", "Phonemic fluency", "Category fluency")
    ...
    output$Figure1<-renderPlot({
      ...
      Data.tot<-structure(list(ID = structure(c(9L, 31L, 18L, 62L, 67L, 22L, 
      ...
      zop<-c()
      for(i in 7:13){
          f<-as.formula(paste0(names(Data.tot)[i+8], "~","Genre+Age+N.etudes+", names(Data.tot)[i]))
          var<-c(names(Data.tot)[i] , names(Data.tot)[i+8], "Genre","Age","N.etudes")
          data.lm<-Data.tot[complete.cases(Data.tot[,var]),]
          sd.m<- sd(data.lm[,names(Data.tot)[i+8]])
          lm.list2[[i-6]]<-lm(f, data.lm )
          R2<-summary(lm.list2[[i-6]])$r.squared
          Xpredit<-predict.lm(lm.list2[[i-6]], NewO, se.fit=T, type="r") 
          zopi<-(T1.subject[i-6]- Xpredit[[1]])/sd.m*(sqrt(1-R2))
          zop<-c(zop, zopi)
     ...
     zop<- round(zop,3)
      ...
      p1<-effect_plot(lm.list2[[1]], pred = T0.SRT.Rappel.moyen,  interval = TRUE)+xlab("T0") + 
      ...
      p2<-effect_plot(lm.list2[[2]], pred = T0.SRT.rappel.diff,  interval = TRUE)+xlab("T0") + 
      ...
      p3<-effect_plot(lm.list2[[3]], pred = T0.Code,  interval = TRUE)+xlab("T0") + 
      ...
      p4<-effect_plot(lm.list2[[4]], pred = T0.Score.Empan.dir,  interval = TRUE)+xlab("T0") + 
      ...
      p5<-effect_plot(lm.list2[[5]], pred = T0.Score.Empan.inv,  interval = TRUE)+xlab("T0") + 
      ...
      p6<-effect_plot(lm.list2[[6]], pred = T0.Fluences.phon,  interval = TRUE)+xlab("T0") + 
      ...
      p7<-effect_plot(lm.list2[[7]], pred = T0.Fluences.cat,  interval = TRUE)+xlab("T0") + 
